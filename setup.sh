...
compile_requirements() {
    local label="$1"
    local requirements_in="$2"
    local requirements_txt="${requirements_in%.in}.txt"

    if [[ ! -f "$requirements_in" ]]; then
        echo -e "${YELLOW}No .in file found for ${label}, using existing .txt${NC}"
        return 0
    fi
    ...
    echo -e "${GREEN}Compiling ${label} dependencies${NC}"
    pip-compile --quiet -c constraints.txt "$requirements_in" --output-file="$requirements_txt"
}
...
with constraints.open('w', encoding='utf-8') as handle:
    ...
    for _, (name, version) in sorted(records.items()):
        # Normalize conda-specific version suffixes for PyPI compatibility
        # e.g., torch 2.5.1.post303 (conda-forge) -> 2.5.1 (PyPI)
        normalized_version = re.sub(r'\.post\d+$', '', version)
        handle.write(f"{name}=={normalized_version}\n")
PY
}
...
compile_requirements "core requirements" "requirements-core.in"

install_requirements "core requirements" "requirements-core.txt"

compile_requirements "LangFlow requirements" "requirements-langflow.in"

install_requirements "LangFlow requirements" "requirements-langflow.txt"

if $WITH_VLLM; then
    compile_requirements "vLLM requirements" "requirements-vllm.in"
